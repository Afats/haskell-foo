...
-- Checking formulas
-- -----------------
--determines whether a given formula is satisfiable
satisfiable :: Formula ts -> Bool
satisfiable (Exists ergo eq) = or $ satisfiable.eq.Con <$> ergo
...
solutions (Exists vs eq) = do 
                  i <- filter (satisfiable.eq.Con) vs
                  j <- (solutions.eq.Con) i
                  (,) <$> pure i <*> pure j